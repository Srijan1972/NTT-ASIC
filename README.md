...
## Dilithium NTT Integration Status

This working tree contains the local Caravel integration for the Dilithium
256-point NTT accelerator. Start with
[`docs/ntt/LOCAL_INTEGRATION.md`](docs/ntt/LOCAL_INTEGRATION.md) for the
Wishbone map, current verification status, and the exact hardening sequence.

This is an integration milestone, not a tapeout-ready result. The NTT macro and
`user_project_wrapper` still need fresh ChipFoundry hardening and signoff before
`cf push`.
