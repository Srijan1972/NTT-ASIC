...
module tb_ntt_core_256_sram;
  ...
  initial begin
    ...
    while (!done && timeout_cycles < 500000) begin
      @(posedge clk);
      timeout_cycles = timeout_cycles + 1;
    end

    if (!done) begin
      $display("FAIL: tb_ntt_core_256_sram timed out");
      $finish;
    end
    ...
    if (errors == 0) begin
      $display("PASS: tb_ntt_core_256_sram");
    end else begin
      $display("FAIL: tb_ntt_core_256_sram errors=%0d", errors);
    end
    $finish;
  end
